"""Costruzione delle statistiche prodotte dall'ingest."""
...
from discovery_io import load_discovery_state, project_path
...
from pipeline_io import load_jsonl, now_iso, relative_path, write_json
...
HTTP_STATUS_RE = re.compile(r"(?:'|status/)([1-5][0-9]{2})(?:\b|[^0-9])", re.IGNORECASE)


def http_failure_kind(error: str) -> str | None:
    """Estrae una categoria HTTP specifica quando il messaggio contiene lo status."""
...
def pdf_failure_kind(record: dict) -> str:
    """Classifica i fallimenti PDF del manifest in categorie leggibili."""
    error_kind = str(record.get("error_kind") or "")
    error = str(record.get("error") or "")
    error_lower = error.lower()

    if record.get("status") == "too_large":
        return "too_large"
    if error_kind == "http_error":
        return http_failure_kind(error_lower) or "http_error"
    if error_kind in {
        ...
        return error_kind
    if error_kind == "download_error":
        if "timeout" in error_lower or "timed out" in error_lower:
            return "timeout"
        return "download_error"
    if "timeout" in error_lower or "timed out" in error_lower:
        return "timeout"
    http_kind = http_failure_kind(error_lower)
    if http_kind:
        return http_kind
    if "bad rect" in error_lower:
        return "extract_failed"
    return error_kind or "other"


def pdf_failures_by_kind(records: list[ProcessedRecord]) -> dict[str, int]:
    """Aggrega i PDF non riusciti nello stato corrente del manifest processed."""
    counts = Counter(
        pdf_failure_kind(record)
        for record in records
        if record.get("source") == "pdf" and record.get("status") in {"failed", "too_large"}
    )
    return {kind: counts[kind] for kind in sorted(counts)}
...
def unique_records_by_url(records: list[dict]) -> list[dict]:
    """Deduplica record preservando l'ultima occorrenza per URL."""
    return list(
        {
...
def sorted_counter(counter: Counter[str]) -> dict[str, int]:
    """Serializza un Counter con ordine stabile."""
    return {key: counter[key] for key in sorted(counter)}


def pdf_keyword_counts(records: list[dict]) -> dict[str, int]:
    """Conta keyword PDF in una lista di record."""
...
def pdf_section_counts(records: list[dict]) -> dict[str, int]:
    """Conta sezioni sorgente PDF in una lista di record."""
    return dict(Counter(pdf_source_section(record) for record in records))


def pdf_coverage_bucket(records: list[dict]) -> dict:
    """Payload ricorrente count/sezione/keyword per i bucket PDF."""
    return {
        "count": len(records),
        "by_section": pdf_section_counts(records),
        "by_keyword": pdf_keyword_counts(records),
    }
...
def build_pdf_coverage(records: list[dict]) -> dict:
    ...
    unique_denied = unique_records_by_url(robots_denied)
    allowed_by_policy = [
        record
        for record in pdf_records
    ...
    unique_allowed = unique_records_by_url(allowed_by_policy)
    suspicious_allowed = [
        record
    ...
    return {
        ...
        "allowed_by_policy": {
            "by_section": pdf_section_counts(unique_allowed),
            "by_keyword": pdf_keyword_counts(unique_allowed),
        },
        "blocked_by_robots": {
            "records": len(robots_denied),
            "unique_pdfs": len(unique_denied),
            "by_section": pdf_section_counts(unique_denied),
            "by_keyword": pdf_keyword_counts(unique_denied),
        },
        "allowed_suspicious_attachments": {
            "count": len(suspicious_allowed),
            "by_section": pdf_section_counts(suspicious_allowed),
            "by_hint": dict(
                Counter(
                    hint
        ...
        "blocked_review_candidates": pdf_coverage_bucket(blocked_by_category["review_candidate"]),
        "blocked_intentionally_excluded": pdf_coverage_bucket(blocked_by_category["intentionally_excluded"]),
        "blocked_other": pdf_coverage_bucket(blocked_by_category["other_blocked"]),
    }
...
def compact_depth_rows(
    ...
    for depth in coverage_depths:
        ...
        row = {
            "depth": depth["depth"],
            "html_recorded_at_depth": html_found_by_depth.get(depth_key, 0),
            "pdf_recorded_at_depth": pdf_found_by_depth.get(depth_key, 0),
            "html_visited_at_depth": visited_html_by_depth.get(depth_key, 0),
...
def compact_seed_rows(
    ...
    return [
        {
            "seed": seed,
            "html_recorded_by_depth": html_found_by_seed_and_depth.get(seed, {}),
            "pdf_recorded_by_depth": pdf_found_by_seed_and_depth.get(seed, {}),
            "html_visited_by_depth": visited_html_by_seed_and_depth.get(seed, {}),
...
def build_processed_stats(
    ...
    pdf_failure_kinds = pdf_failures_by_kind(records)
    if pdf_failure_kinds:
        report["errors"] = {
            "pdf_failures": {
                "by_kind": pdf_failure_kinds,
            },
        }
    return report
